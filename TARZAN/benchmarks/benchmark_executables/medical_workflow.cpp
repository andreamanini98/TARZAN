 ...
 */
inline void testMedicalWorkflow(const std::string &path, const std::string &benchmarkKey)
{
    ...
    const int numPatients = std::stoi(benchmarkKey) * 10 + 10;
    const int numElements = numPatients + 2;

    auto goal = std::vector<std::optional<int>>(numElements, std::nullopt);
    for (int i = 2; i < numElements; i++)
        goal[i] = locationsToInt[i].at("Done");
...
int main(const int argc, char *argv[])
{
    ...
    // Query: E<> ( every patient done )
    testMedicalWorkflow(path, benchmarkKey);
